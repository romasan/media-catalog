...
App
└── AppContent (observer)
        ├── FilterBar           — панель фильтра по тегам + счётчик фото/видео
...
|-----------|------|-----------|
| `MediaGrid` | `components/MediaGrid/MediaGrid.tsx` | Виртуализированная сетка: число колонок = `floor(ширина контейнера / 100)`, размер ячейки = `ширина / колонок` — карточки полностью заполняют ширину окна в диапазоне 100–200 px. Рендерит только видимые строки (overscan 3 строки). Использует `ResizeObserver` для отслеживания размеров. Также вычисляет индексы карточек, реально попадающих в область просмотра (без overscan), и передаёт их диапазон дат в `DateRangeLabel`. |
| `MediaCard` | внутри `MediaGrid` | Карточка превью. Для видео показывает бейдж ▶. Грузит превью через `window.api.getMediaStreamUrl`. |
| `DateRangeLabel` | `components/DateRangeLabel/DateRangeLabel.tsx` | Подпись в правом верхнем углу с диапазоном дат (`modifiedAt`) карточек в видимой области сетки; обновляется при скролле. Детализация зависит от длины диапазона: тот же день — `22 апреля 2025`; тот же месяц — `1 - 9 мая 2025`; тот же год — `1 июня - 30 июля 2025`; разные годы — `20 декабря 2025 - 7 января 2026`. Не перехватывает клики (`pointer-events: none`). |
| `FilterBar` | `components/FilterBar/FilterBar.tsx` | Панель выбранных тегов + переключатель «все»/«любой» (AND/OR) (виден при >1 теге). Метатеги отображаются серыми чипами. Панель видна всегда; справа от тегов показывается счётчик фото/видео (`📷 N 🎬 M`) для текущей выдачи: при активном фильтре — с учётом фильтра, при пустом — общее количество записей в каталоге. |
...
| `ThumbnailProgressBar` | `components/ThumbnailProgressBar/ThumbnailProgressBar.tsx` | Показывает прогресс генерации превью (%), скрывается через 300 мс после завершения. Расположен в правом верхнем углу под подписью `DateRangeLabel` (`top: 56px`), чтобы не перекрывать её. |
